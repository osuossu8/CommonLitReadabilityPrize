...
class RoBERTaLarge(nn.Module):
    def __init__(self, model_path):
        ...
        self.l0 = nn.Linear(self.in_features * 3 + 8 + 32, 1)
        self.l1 = nn.Linear(self.in_features * 3 + 8 + 32, 12)
    ...
    def forward(self, ids, mask, numerical_features, tfidf):
        ...
        ro = roberta_outputs[0] # bs, 256, 1024
        ft = ro[:, 0, :]
        lt = ro[:, -1, :]
        ...
        x = torch.cat([x1, x2, x3, ft, lt], 1) # bs, 1024 + 8 + 32
