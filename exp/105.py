...
class CommonLitDataset:
    def __init__(self, df, excerpt, tokenizer, max_len, numerical_features, tfidf, w2v):
    ...
    def __getitem__(self, item):
        ...
        return {
            ...
            "w2v" : torch.tensor(w2v, dtype=torch.float32),
        }
...
def train_fn(epoch, model, train_data_loader, valid_data_loader, device, optimizer, scheduler, best_score):
    ...
    for batch_idx, data in enumerate(tk0):
        ...
        outputs, aux_outs = model(inputs, masks, numerical_features, tfidf, w2v)
...
def calc_cv(model_paths):
    ...
    for fold, model in enumerate(models):
        ...
        dataset = CommonLitDataset(df=val_df, excerpt=val_df.excerpt.values, tokenizer=tokenizer, max_len=CFG.max_len, numerical_features=df[CFG.numerical_cols].values, tfidf=tfidf_df, w2v=w2v_vecs)
        ...
        for b_idx, data in tqdm(enumerate(data_loader)):
            with torch.no_grad():
                ...
                tfidf = data['tfidf'].to(device)
                w2v = data['w2v'].to(device)
                output, _ = model(inputs, masks, numerical_features, tfidf, w2v)
...
def w2v_vectorize(model, word_list : list) -> np.array:
    V = []
    for word in word_list:
        try:
            vector = model.wv[word]
        except:
            vector = [0 for j in range(50)]
        V.append(vector)
    return np.mean(V, 0)
...
for fold in range(5):
    ...
    train_dataset = CommonLitDataset(df=trn_df, excerpt=trn_df.excerpt.values, tokenizer=tokenizer, max_len=CFG.max_len, numerical_features=trn_df[CFG.numerical_cols].values, tfidf=tfidf_df, w2v=w2v_vecs)
    train_dataloader = torch.utils.data.DataLoader(
        train_dataset, batch_size=CFG.train_bs, num_workers=0, pin_memory=True, shuffle=True
    )
    
    valid_dataset = CommonLitDataset(df=val_df, excerpt=val_df.excerpt.values, tokenizer=tokenizer, max_len=CFG.max_len, numerical_features=val_df[CFG.numerical_cols].values, tfidf=tfidf_df, w2v=w2v_vecs)
