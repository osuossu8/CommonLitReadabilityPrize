...
class RoBERTaLarge(nn.Module):
    ...
    def forward(self, ids, mask, tfidf):
        ...
        logits = self.l0(self.dropout(x)) + self.l1(self.dropout1(tfidf))
...
tfidf_df = pd.DataFrame(z, columns=[f'cleaned_excerpt_tf_idf_svd_{i}' for i in range(64*2)])
...
for fold in range(5):
    ...
    train_dataset = CommonLitDataset(df=trn_df, excerpt=trn_df.excerpt.values, tokenizer=tokenizer, max_len=CFG.max_len, tfidf=tfidf_df)
    train_dataloader = torch.utils.data.DataLoader(
        train_dataset, batch_size=CFG.train_bs, num_workers=0, pin_memory=True, shuffle=True
    )
    
    valid_dataset = CommonLitDataset(df=val_df, excerpt=val_df.excerpt.values, tokenizer=tokenizer, max_len=CFG.max_len, tfidf=tfidf_df)
