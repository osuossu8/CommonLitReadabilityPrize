...
class CommonLitDataset:
    def __init__(self, df, excerpt, tokenizer, max_len, numerical_features, tfidf, use_features):
...
class RoBERTaLarge(nn.Module):
    def __init__(self, model_path):
        ...
        self.linear = nn.Linear(self.in_features + 8 + 32 + 512, 256)
        self.relu = nn.ReLU()
        self.l0 = nn.Linear(256, 1)
        self.l1 = nn.Linear(256, 7)

    def forward(self, ids, mask, numerical_features, tfidf, use_features):
        ...
        x = torch.cat([x1, x2, x3, use_features], 1) # bs, 1024 + 8 + 32 + 512
...
def train_fn(epoch, model, train_data_loader, valid_data_loader, device, optimizer, scheduler, best_score):
    ...
    for batch_idx, data in enumerate(tk0):
        ...
        outputs, aux_outs = model(inputs, masks, numerical_features, tfidf, use_features)
...
def calc_cv(model_paths):
    ...
    for fold, model in enumerate(models):
        ...
        dataset = CommonLitDataset(df=val_df, excerpt=val_df.excerpt.values, tokenizer=tokenizer, max_len=CFG.max_len, 
                                   numerical_features=df[CFG.numerical_cols].values, tfidf=tfidf_df, use_features=df[CFG.USE_cols].values
                                  )
...
for fold in range(5):
    ...
    train_dataset = CommonLitDataset(df=trn_df, excerpt=trn_df.excerpt.values, tokenizer=tokenizer, max_len=CFG.max_len, 
                                     numerical_features=trn_df[CFG.numerical_cols].values, tfidf=tfidf_df, use_features=trn_df[CFG.USE_cols].values
                                    )
    ...
    valid_dataset = CommonLitDataset(df=val_df, excerpt=val_df.excerpt.values, tokenizer=tokenizer, max_len=CFG.max_len, 
                                     numerical_features=val_df[CFG.numerical_cols].values, tfidf=tfidf_df, use_features=val_df[CFG.USE_cols].values
                                    )
