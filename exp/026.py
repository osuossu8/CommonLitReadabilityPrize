...
def make_model(model_name=CFG.itpt_path, num_labels=1):
...
def config(fold=0):
    ...
    model, tokenizer = make_model(model_name=CFG.itpt_path, num_labels=1)
...
def run(fold=0):
    ...
    import time
    trainer = Trainer(model, optimizer, scheduler, scaler, log_interval=10, evaluate_interval=10)
    train_time_list = []
