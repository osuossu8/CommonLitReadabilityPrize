...
def train_fn(epoch, model, train_data_loader, valid_data_loader, device, optimizer, scheduler, best_score):
    ...
    for batch_idx, data in enumerate(tk0):
        ...
        outputs2, _ = model(inputs, masks, numerical_features, tfidf)
        kl_loss = compute_kl_loss(outputs, outputs2)
        loss = loss_fn(outputs, targets) * 0.25 + loss_fn(outputs2, targets) * 0.25 + aux_loss_fn(aux_outs, aux_targets) * 0.25 + kl_loss * 0.25
        loss.backward()
        optimizer.step()
        # scheduler.step()
